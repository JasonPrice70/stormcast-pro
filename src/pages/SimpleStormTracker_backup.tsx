  ...
  return (
    <div className="simple-storm-tracker">
      ...
      <div className="map-wrapper">
        ...
        {displayStorms.map((storm) => (
          <Marker 
            ...
            icon={createStormIcon(storm.category || 0, storm.classification)}
            eventHandlers={{
              click: () => {
                console.log('Storm marker clicked:', storm.name, storm);
              }
            }}
          >
            <Popup>
              <div className="storm-popup">
                ...
                <div className="storm-popup-content">
                  ...
                  <p className="storm-popup-field">
                    <strong>Category:</strong> {storm.category > 0 ? `Category ${storm.category}` : 'N/A'}
                  </p>
                  
                  <p className="storm-popup-field">
                    <strong>Max Winds:</strong> <span className="storm-popup-winds">{storm.maxWinds} mph</span>
                  </p>
                  ...
                  <p className="storm-popup-field">
                    <strong>Position:</strong> {storm.position[0].toFixed(1)}°N, {Math.abs(storm.position[1]).toFixed(1)}°W
                  </p>
                  
                  <p className="storm-popup-last-update">
                    <strong>Last Update:</strong> {storm.lastUpdate.toLocaleString()}
                  </p>
                  
                  {storm.advisoryUrl && storm.advisoryUrl !== '#' && (
      ...
      <div className="floating-control-panel">
        <h4 className="control-panel-header">
          {hasStorms 
            ? `${dataSource === 'demo' ? 'Demo ' : ''}Storms: ${displayStorms.length}`
            : 'Storm Status'
          }
        </h4>
        <div className="control-panel-content">
          {loading ? (
            <span className="control-panel-loading">Loading...</span>
          ) : error && !useDemo ? (
            <>
              <span className="control-panel-error">API Error</span>
              <button 
